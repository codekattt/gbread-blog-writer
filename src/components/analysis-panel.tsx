    ...
    <section className="section-card p-5 sm:p-6">
      ...
      <div className="mt-6 grid gap-4 xl:grid-cols-[1.05fr_0.95fr]">
        <article className="rounded-[1.5rem] border border-[var(--color-line)] bg-white p-5">
          ...
          <h3 className="mt-3 text-xl font-semibold leading-8 text-[var(--color-ink)]">
            {analysis.video.title}
          </h3>
          <p className="mt-4 text-sm leading-7 text-[var(--color-muted)]">{analysis.analysis.summary}</p>
        ...
        <div className="grid gap-4">
          <article className="rounded-[1.5rem] border border-[var(--color-line)] bg-white p-5">
            ...
            <ul className="mt-4 space-y-3 text-sm leading-6 text-[var(--color-ink)]">
              {analysis.analysis.keyPoints.map((point) => (
                <li key={point} className="rounded-[1rem] bg-[rgba(248,250,252,0.9)] px-4 py-3">
                  {point}
                </li>
              ))}
            </ul>
      ...
      <div className="mt-4 grid gap-4 lg:grid-cols-[0.9fr_1.1fr]">
        <article className="rounded-[1.5rem] border border-[var(--color-line)] bg-white p-5">
          <p className="text-xs uppercase tracking-[0.22em] text-[var(--color-muted)]">Outline</p>
