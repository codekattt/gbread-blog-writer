    ...
    <section className="section-card p-5 sm:p-6">
      ...
      <div className="mt-6 grid gap-4 xl:grid-cols-[0.9fr_1.1fr]">
        <article className="rounded-[1.5rem] border border-[var(--color-line)] bg-white p-5">
          ...
          <div className="mt-5 rounded-[1.25rem] bg-[rgba(239,246,255,0.84)] p-4">
            <p className="text-xs uppercase tracking-[0.18em] text-[var(--color-muted)]">Summary</p>
            <p className="mt-2 text-sm leading-6 text-[var(--color-ink)]">{draft.summary}</p>
          </div>
          <div className="mt-5 rounded-[1.25rem] bg-[rgba(240,253,244,0.88)] p-4">
            <p className="text-xs uppercase tracking-[0.18em] text-[var(--color-muted)]">Hashtags</p>
            <div className="mt-2 flex flex-wrap gap-2">
              {draft.hashtags.map((tag) => (
                <span
                  key={tag}
                  className="rounded-full border border-[rgba(22,101,52,0.14)] bg-white px-3 py-1 text-xs font-medium text-[var(--color-success)]"
                >
                  {tag}
                </span>
              ))}
        ...
        <article className="rounded-[1.5rem] border border-[var(--color-line)] bg-white p-5">
          <p className="text-xs uppercase tracking-[0.22em] text-[var(--color-muted)]">Preview</p>
          <div className="mt-4 space-y-5">
            <div>
              <h3 className="text-2xl font-semibold leading-9 text-[var(--color-ink)]">
                {draft.selectedTitle}
              </h3>
              <p className="mt-3 text-sm leading-7 text-[var(--color-muted)]">{draft.openingParagraph}</p>
            </div>
            ...
            <div className="space-y-4">
              {draft.sections.map((section) => (
                <div
                  key={section.heading}
                  className="rounded-[1.25rem] border border-[var(--color-line)] bg-[rgba(248,250,252,0.74)] p-4"
                >
            ...
            <div className="rounded-[1.25rem] bg-[rgba(255,247,237,0.72)] p-4">
              <p className="text-sm leading-7 text-[var(--color-ink)]">{draft.closingParagraph}</p>
              <p className="mt-3 text-sm font-semibold text-[var(--color-accent)]">{draft.cta}</p>
            </div>
          </div>
        </article>
      </div>
